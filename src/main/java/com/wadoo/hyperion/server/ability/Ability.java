package com.wadoo.hyperion.server.ability;
...
import com.wadoo.hyperion.server.ability.AbilitySection.AbilitySectionInstant;
import com.wadoo.hyperion.server.ability.AbilitySection.AbilitySectionDuration;
...
public class Ability<T extends LivingEntity> {
    ...
    public void playAnimation(RawAnimation animation) {
        if (getUser() instanceof HyperionMob && getUser().level().isClientSide()) {
            HyperionMob entity = (HyperionMob) getUser();
            activeAnimation = animation;
            AnimationController<GeoAnimatable> controller = entity.getAnimatableInstanceCache().getManagerForId(entity.getId()).getAnimationControllers().get("controller").transitionLength(0);
            if (controller != null) {
     ...
     * @return Whether the ability try succeeded
     */
